...
if (!exists('maindf')) {
  training<-read.csv(textConnection(xURL_training))
}

if (!exists('testdf')) {
  testing<-read.csv(textConnection(xURL_testing))
}
...
cm1<-confusionMatrix(predictionsB1, myTesting$classe)

predictionsB2 <- predict(modFitB1, testing, type = "class")
cm2<-confusionMatrix(predictionsB1, myTesting$classe)
